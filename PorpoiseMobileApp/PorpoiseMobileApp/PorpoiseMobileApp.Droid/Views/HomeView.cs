...
namespace PorpoiseMobileApp.Droid.Views
{
    ...
    public class HomeView : MvvmAppCompatActivity<HomeViewModelAndroid>, IFragmentHost
    {
		...
		private MenuItem viewId;



		public ImageButton BtnLogHour { 
		
			get {

				return this.btnLogHour;
			
			}
			set {

				this.btnLogHour = value;
			}
        ...
        protected override void OnCreate(Bundle bundle)
        {
			...
			Intercom.Initialize(this.Application, "android_sdk-b004c845f098668ca5dfc782235ca1358cf82a92", "f1xl6ben");
			//Intercom.Initialize(this.Application, "", "f1xl6ben");
		...
		public void clearButtons() { 
			...
			btnProfile.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
			btnActivity.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
			btnLogHour.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
			btnSetting.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
			btnSupport.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
		...
		private async void LoadMenuButtons()
		{
			...
			btnProfile.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_orange)));
			btnActivity.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
			btnLogHour.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
			btnSetting.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
			btnSupport.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
			...
			{
				Console.WriteLine("REDIRECTING TO PROFILE VIEW");
				Show(new MvxViewModelRequest(typeof(ProfileViewModel), null, null, null));
				//btnProfile.Drawable.SetColorFilter(Color.Orange, PorterDuff.Mode.);
				btnProfile.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_orange)));
				btnActivity.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnLogHour.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnSetting.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
			...
			btnActivity.Click += (s, g) =>
			{
				...
				btnActivity.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_orange)));
				btnProfile.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnLogHour.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnSetting.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnSupport.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
			...
			btnLogHour.Click += (s, g) =>
			{
				...
				btnLogHour.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_orange)));
				btnActivity.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnProfile.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnSetting.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnSupport.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
			...
			btnSetting.Click += (s, g) =>
			{
				...
				btnSetting.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_orange)));
				btnActivity.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnLogHour.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnProfile.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnSupport.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
			...
			btnSupport.Click += (s, g) =>
			{
				...
				dict.Add("email", AccountInfo.Email);
				dict.Add("name", employee.Payload.FirstName+" "+employee.Payload.LastName);
				Intercom.Client().UpdateUser(dict);
				Intercom.Client().DisplayMessenger();
				//btnSupport.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_orange)));
				btnActivity.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnLogHour.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnSetting.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_gray)));
				btnProfile.BackgroundTintList = (ColorStateList.ValueOf(Resources.GetColor(Resource.Color.post_orange)));
        ...
        protected override void OnViewModelSet()
        {
            ...
            */
			...
			//Bindings.Bind(logout).To(vm => vm.LogoutCommand);
            //Bindings.Apply();
            ...
            if (null == _savedState)
            {
                this.ViewModel.ShowMenuItem(this.ViewModel.MenuItems[1]);
            }
        ...
        public bool Show(MvxViewModelRequest request)
        {
            try
            {
                ...
                {
					foreach (KeyValuePair<MenuItem,System.Type> aux in ViewMappings) {

						Console.WriteLine("MENU ITEM: "+aux.Key.ToString());

					}
                    //Console.WriteLine("MenuItem Selected " + menuItem.MenuItem.ToString());

                    if(menuItem != null){
						this.viewId = menuItem.MenuItem;
						switch (viewId) { 
						...
						System.Console.WriteLine("MENU ITEM IS NOT NULL "+this.viewId);
						if (menuItem.MenuItem != null && ViewMappings != null && ViewMappings.ContainsKey(menuItem.MenuItem))
						{
        ...
        private void HandleDirty(Action proceed)
        {
            ...
            if (current != null && current is IDirty && !handlingDirty)
            {
                ...
                {
                    handlingDirty = true;
                    this.Alert(PorpoiseMobileApp.Droid.Extensions.AlertType.Confirm, PorpoiseMobileApp.Resource.AreYouSure, PorpoiseMobileApp.Resource.UnsavedChanges, whichButton =>
                    {
                        try
                        {
    ...
    internal class BackStackListener<TViewModel> : Java.Lang.Object, Android.Support.V4.App.FragmentManager.IOnBackStackChangedListener where TViewModel : MvxViewModel
    {
        ...
        public void OnBackStackChanged()
        {
            if (BackStackChanged != null)
            {
                ...
                {
                    var args = new BackStackEventArgs { BackStackCount = ((Android.Support.V7.App.AppCompatActivity)activity).SupportFragmentManager.BackStackEntryCount };
                    BackStackChanged(activity, args);
                }
            }
        }
