...
using PorpoiseMobileApp.iOS.Custom;
    ...
    public partial class LogHourPhotoViewController : MvvmViewController<LogHourPhotoViewModel>
    {
        ...
        protected void Handle_FinishedPickingMediaCamera(object sender, UIImagePickerMediaPickedEventArgs e)
        {
            ...
            if (isImage)
            {
                // get the original image
                originalImage = e.Info[UIImagePickerController.OriginalImage] as UIImage;
                originalImage = PorpoiseImage.resizeImage(originalImage, 200);
                //originalImage = PorpoiseMobileApp.iOS.Custom.PorpoiseImage.correctlyOrientedImage(originalImage);

                Debug.WriteLine("IMAGE ORIENTATION: " + originalImage.Orientation);
                UIImage image=null;
                switch (originalImage.Orientation)
				{
                    case UIImageOrientation.Down:
                        image = new UIImage(originalImage.CGImage, originalImage.CurrentScale, UIImageOrientation.Right);
						break;
				...
				Debug.WriteLine("IMAGE ORIENTATION: " + image.Orientation);
                originalImage = image;
                //originalImage.GetImageWithHorizontallyFlippedOrientation();
                if (originalImage != null)
                {
                    imageView.Image = this.originalImage;
                    // do something with the image
        ...
        {
            UzysAssetsPickerController.UzysAssetsPickerController picker = new UzysAssetsPickerController.UzysAssetsPickerController();
			//UzysAssetsPickerController.UzysAssetsPickerController picker = new UzysAssetsPickerController.UzysAssetsPickerController();
            ...
            this.mainView.AddSubview(picker.View);
			picker.View.Frame = new CGRect(this.mainView.Bounds.X, this.mainView.Bounds.Y, this.mainView.Bounds.Width, this.mainView.Bounds.Height);
            ...
            UzysAssetsPickerController.UzysGroupPickerView groupPicker = picker.GroupPicker;
			//UzysAssetsPickerController.UzysGroupPickerView groupPicker = picker.GroupPicker;



			picker.BtnDone.TouchUpInside += (sender, e) => {

				Debug.WriteLine("IMAGE SELECTED");
				long size = picker.SelectedAsset.DefaultRepresentation.Size;
				Console.WriteLine("Filename: " + picker.SelectedAsset.DefaultRepresentation.Filename);
    			byte[] imgData = new byte[size];
				NSError nsError;
				IntPtr buffer = Marshal.AllocHGlobal(imgData.Length);

				picker.SelectedAsset.DefaultRepresentation.GetBytes(buffer, 0, (uint)size, out nsError);


   
    			Marshal.Copy(buffer, imgData, 0, imgData.Length);

				Debug.WriteLine("New Image Size: " + imgData.Length);

				LogPost.image = imgData;
			...
			Debug.WriteLine("SOURCE TYPE ABVAILABLE: "+UIImagePickerController.IsSourceTypeAvailable(UIImagePickerControllerSourceType.PhotoLibrary));
		...
		void NavigationBarSetUp()
		{
			...
			if(!LogPost.isEditing){
				...
				if (scale >= 2)
				{
                   ...
                   cancel = Services.PorpoiseImage.getFromURL("https://s3.amazonaws.com/porpoise-cdn/mobile-assets/Back%402x.png");

				}
				else
				{
                   cancel = Services.PorpoiseImage.getFromURL("https://s3.amazonaws.com/porpoise-cdn/mobile-assets/back.png");
            ...
            cancelButton.Bounds = new CGRect(0, 0, Services.PorpoiseImage.getAppropriateWidth(cancel, (nfloat)(NavigationController.NavigationBar.Bounds.Height * 0.5)), NavigationController.NavigationBar.Bounds.Height * 0.5);
